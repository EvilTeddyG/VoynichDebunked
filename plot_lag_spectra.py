...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--top-controls", type=int, default=8, help="Max non-Voynich corpus lines in comparative plot")
...
def plot_comparative(rows, summary, outpath, top_controls=8):
    ...
    for label, vals in by_label.items():
        if label == "voynich":
            continue
        if nearest and label not in nearest:
            continue
...
def plot_family_aggregate(rows, outpath):
    ...
    for family, lag_map in sorted(by_family_lag.items()):
        ...
        low = [m - s for m, s in zip(mu, sd)]
        high = [m + s for m, s in zip(mu, sd)]
